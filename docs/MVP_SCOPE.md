...
7. **Триггеры обновления ленты:**
   - **Первый прогон** — автоматически когда в system feed появляется ≥1 сигнал (`trigger=initial`);
   - **Ingest** — кнопка «Обновить ленту» (`POST /api/ideas/ingest`);
   - **Повторный анализ** — «Обновить идеи» на research detail (`POST …/analyze`, `trigger=manual`);
